...
map<string, string> cloneLevelMap;
AllData allData();
...
void iniCloneLevelMap(){
    cloneLevelMap["function"] = "methodDeclaration";
}
...
void getFTFromSA(vector<unsigned char>& token_data, vector<int>& token_label, vector<int>& sa, int s, int e, int& cntt, vector<FiveTuple>& ft, int sa_len) {
    for (int i = s; i < e; i++) {
        ...
        for (int k=i+1; k < min(sa_len, i+100); k++) {
            int s2 = sa[k];
...
int main(int argc, char* argv[]){
    ...
    ofstream outputFile;
    outputFile.open("./output/"+cloneLV+"2.txt");
    ...
    ofstream dataFile;
	dataFile.open("./output/allFunctions.csv", ios::out | ios::trunc);
    ...
    ofstream cloneFile;
	cloneFile.open("./output/clonePairs.csv", ios::out | ios::trunc);
    for(auto iter = notAstClonePairs.begin(); iter != notAstClonePairs.end(); iter++) {
        set<int> tmpSet = iter->second;
        int f_id = iter->first;
